  ...
  const fetchTasks = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_SERVER_URL}/tasks?userId=${user._id}`
      )
  ...
  const handleCompleteTask = async (task) => {
    try {
      const respone = await axios.put(
        `${process.env.REACT_APP_SERVER_URL}/tasks/${task._id}/complete`
      )
            ...
            {!isViewingTaskDetail ? (
              ...
              currentTask && (
                <div>
                  ...
                  <button
                    className="z-0 group relative inline-flex items-center justify-center px-4 min-w-20 h-10 text-small gap-2 rounded-xl bg-gray-400 text-white hover:opacity-hover mt-4"
                    type="button"
                    onClick={() => setIsViewingTaskDetail(false)}
                  >
                    Back
